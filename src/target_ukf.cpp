...
#include <tracking_experiment/Trajectory3D.h>
...
void dynamics_cb(const tracking_experiment::Dynamics::ConstPtr& msg)
{
...
void features_cb(const tracking_experiment::CameraFeatures::ConstPtr& msg)
{
    cam1.fx = msg->fx.data;
    cam1.fy = msg->fy.data;
    cam1.cu = msg->cu.data;
...
void qp_cb(const tracking_experiment::Trajectory3D::ConstPtr& msg)
{
    qp1.acc[0] = msg->acc.x;
    qp1.acc[1] = msg->acc.y;
    qp1.acc[2] = msg->acc.z;
    ...
    //cout << "msg->pos.x:  " << msg->pos.x << "  " << isnan(msg->pos.x) << endl;
...
*/
    ...
    if(!measurement_flag && qp_flag)
    {
        cout << "qp -> Yolo lost.--------" << endl;
    }
    if(!measurement_flag && !qp_flag)
    {
        cout << "easy -> constant model.--------" << endl;
    }
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Subscriber dynamics_sub = nh.subscribe<tracking_experiment::Dynamics>("estimation/ukf/dynamics", 10, dynamics_cb);
    ros::Subscriber gimbal_sub = nh.subscribe<tracking_experiment::CameraFeatures>("estimation/ukf/camera_features", 10, features_cb);
    ros::Subscriber qp_sub = nh.subscribe<tracking_experiment::Trajectory3D>("/target_qp1", 10, qp_cb);
    ros::Subscriber car_sub = nh.subscribe<nav_msgs::Odometry>("/prius/pose_ground_truth", 10, car_cb);
    ros::Publisher estimate_pub = nh.advertise<tracking_experiment::EstimateOutput>("estimation/ukf/output_data", 1);
    ros::Publisher gt_pub = nh.advertise<tracking_experiment::EstimateOutput>("estimation/ukf/groundtruth", 1);
    ...
    tracking_experiment::EstimateOutput gt_value, estimate_value;
